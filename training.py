...
args = Namespace(dataset_csv="data/inp_and_gt_name_near_food_no_inform.csv",
                 vectorizer_file="checkout_on_best_energy_4_yes_sparse.json",
                 model_state_file="checkout_on_best_energy_4_yes_sparse.pth",
...
with open("training_monitor.txt", "a") as f:
            f.write("Bahdanau Attention, Sparsemax, 48, 48, 256, no inform, softmax, energy loss 4, Renyi")
            f.write("\n")
...
try:
    for epoch_index in range(args.num_epochs):
        ...
        for batch_index, batch_dict in enumerate(batch_generator):
            ...
            caps = np.ones((at_energies.size()[0],  at_energies.size()[1]), dtype=np.float32)*4.
